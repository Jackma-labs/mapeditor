...
function getIssueGuide(issue: MapQualityIssue): IssueGuide {
    ...
    if (text.includes('没有前驱') || text.includes('疑似缺少前驱')) {
        return {
            title: '疑似缺少前驱怎么处理',
            steps: [
                '质检只在附近存在候选车道、距离和方向都像断点时提示；没有候选的合法入口不再报警。',
                '如果候选车道确实是上一段路线，优先点“智能修复”，或选中两条车道后用“直道连接/弯道连接”。',
                '如果这里就是地图入口，可以忽略该警告，但发布前要确认主路线不会从这里断开。',
            ],
        };
    }
    if (text.includes('没有后继') || text.includes('疑似缺少后继')) {
        return {
            title: '疑似缺少后继怎么处理',
            steps: [
                '质检只在附近存在候选车道、距离和方向都像断点时提示；没有候选的合法出口不再报警。',
                '如果候选车道确实是下一段路线，优先点“智能修复”，或选中两条车道后用“直道连接/弯道连接”。',
                '如果这里就是地图出口，可以忽略该警告，但发布前要确认闭环路线不会在这里停死。',
            ],
        };
    }
    ...
    if (text.includes('疑似断点') || text.includes('拓扑') || text.includes('孤立')) {
        return {
            ...
            steps: [
                ...
                '如果是地图边界入口/出口，可以保留，但要确认它不会阻断主路线。',
            ],
        };
    }
